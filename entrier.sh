...
if [ -d "$DIR" ]; then
    ...
    rm -f changes;
    echo $'\n'"git status: $(git status)"$'\n';
    git config --global user.name arshvirkdirba && git config --global user.email arshvirkdirba@gmail.com
    git add . && git commit -m "Art Entry: $(date)"
    echo $'\n'"git status: $(git status)"$'\n';
    git push "https://arshvirkdirba:$GH_PAT@github.com/arshvirkdirba/arshvirkdirba.github.io.git" HEAD:master
else
